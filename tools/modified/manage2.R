...
output$ui_Manage <- renderUI({
  list(
    wellPanel(
      actionButton('loadExampleData', 'Load full dataset'),
      h6("This may take a few minutes, yes yes wait"),
      conditionalPanel( condition = "input$loadExampleData == -40 && $('html').hasClass('shiny-busy')","working, do the dance..." )
      #radioButtons(inputId = "dataType", label = "Load data:", c("examples" = "examples"), selected = "examples"),
      #  selected = "rda"),
...
# loading all examples files  to dropdown
loadUserDataFileName <- function(filename, uFile, ext) {
  ...
  if(ext == 'rda') {
    ...
    if(length(objname) > 1) {
      ...
      values[[objname]] <- as.data.frame(get(objname[-which(objname == "description")]))
      values[[paste0(objname,"_descr")]] <- get("description")
    } else {
      values[[objname]] <- as.data.frame(get(objname))
...
observe({
  ...
  isolate({
    ...
    print(ex)
		#filename <- ex
		#objname <- sub(paste(".","rda",sep = ""),"",basename(filename))
		#values$datasetlist <- c(objname)
		loadUserDataFileName(ex,paste0(path,ex), 'rda')
	}
    ...
    #textOutput("after load")
    filename <- "data/data_init/SnapshotGoogleDataset.csv"#full_adjusted_Id.csv
    objname <- sub(paste(".","csv",sep = ""),"",basename(filename))
    values[[objname ]] <- read.csv(filename, header=TRUE, sep=",")
    values$datasetlist <- c(objname)
    #textOutput("before load")
