...
output$ui_Manage <- renderUI({
	values$sampleMode <- FALSE
	print(paste("val man",values$sampleMode))
  list(
	  wellPanel(
      uiOutput("uiFullTestsList"),
      actionButton('loadSelectedFile', 'Load selected file')
      ),
	  wellPanel(
      radioButtons(inputId = "dataType", label = "Load data drom disk:", c(".rda" = "rda", ".csv" = "csv", "clipboard" = "clipboard"), 
        selected = "rda"),
    ...
    wellPanel(
      ...
      conditionalPanel(condition = "input.saveAs != 'clipboard' && input.man_add_descr == false",
        downloadButton('downloadData', 'SaveNow')
      )
    ),
    #wellPanel(
...
observe({
  ...
  isolate({
	...
	print(paste("user selected file in index:",ind))
...
output$uiRemoveDataset <- renderUI({
  ...
  dataSetsInMemory <- NULL
  for (set in values$datasetlist){
	datSet <- values[[set]]			
	if ( is.null(datSet) == FALSE ) {
 	dataSetsInMemory <- c(dataSetsInMemory,set)
	}
  }
  selectInput(inputId = "removeDataset", label = "Remove data from memory:", 
    choices = dataSetsInMemory, selected = NULL, multiple = TRUE, selectize = FALSE)
})

# removing datasets
output$uiFullTestsList <- renderUI({
  # Drop-down selection of data set to remove
  
	path <- "data/googleTestWithTestSuit/"
	examples <- list.files(path)
	listTestFiles <- NULL
    for(ex in examples) {
		objname <- sub(paste(".","csv",sep = ""),"",basename(ex))
		listTestFiles <- c(objname,listTestFiles)
	}	
	
  selectInput(inputId = "fullTestsList", label = "List Of All Tests:", 
    choices = listTestFiles, selected = NULL, multiple = TRUE, selectize = FALSE)
})
...
observe({
  # add datasets for tests list
  if(is.null(input$loadSelectedFile) || input$loadSelectedFile == 0) return()
  isolate({

    datasetToLoad <- input$fullTestsList  
    for(fileToLoad in datasetToLoad) {
      path <- "data/googleTestWithTestSuit/"
	  ex <- fileToLoad
	  loadUserData(ex, paste0(path,ex,".csv"), 'csv')
    }
        #datasets <- datasets[-which(datasets %in% removeDataset)]
		#values[['datasetlist']] <- datasets
...
observe({
	print("visit")
	if(is.null(input$downloadData) || input$downloadData == 0) return()
	isolate({
		print(paste("file:",input$downloadData))
	})
})
...
output$downloadData <- downloadHandler(
  filename = function() { 
  paste(input$datasets,'.',input$saveAs, sep='') 
  print(paste(input$datasets,'.',input$saveAs, sep=''))
  },
  content = function(file) {
	#print(paste("file:",input$downloadData)) #['outputId']
    ext <- input$saveAs
    robj <- input$datasets
	#file = input$downloadData
	print(paste("fname:",file))
    if(ext == 'rda') {
      ...
      assign(robj, getdata())
	  #print(get(robj))
	  write.csv(get(robj), file)#v"C:\\Users\\Nimrod\\Dropbox\\googleProject\\b.csv"
    }
  }
)
...
loadFileName <- function(filename, uFile, ext) {
...
#	ex <- input$datasets
#	loadUserData(ex, paste0(path,ex,".csv"), 'csv')
	#loadUserData(filename, uFile, ext) (ex, paste0(path,ex), 'rda')
 # }
