...
output$ui_Visualize <- renderUI({
	list(wellPanel(
	  ...
	  checkboxInput('viz_failed', 'Only display failed test', value = state_init("viz_failed", FALSE)),
...
.visualize <- reactive({
	...
	visualize(input$datasets, input$vizvars1, input$vizvars2,input$viz_failed, input$viz_select,input$viz_multiple, input$viz_facet_row, 
...
observe({
  ...
  isolate({
		inp <- list(input$datasets, input$vizvars1, input$vizvars2,input$viz_failed, input$viz_select, input$viz_multiple, input$viz_facet_row, 
			input$viz_facet_col, input$viz_color, input$viz_line, input$viz_loess, input$viz_jitter)
		updateReportViz(inp,"visualize", round(7 * viz_plot_width()/650,2), round(7 * viz_plot_height()/650,2))
  })
})

visualize <- function(datasets, vizvars1, vizvars2,viz_failed, viz_select, viz_multiple, viz_facet_row, viz_facet_col,
  ...
  if(viz_failed) {
    seldat <- try(do.call(subset, list(dat,parse(text = "Status == 'FAILED'"))), silent = TRUE)
    if(!is(seldat, 'try-error')) {
      if(is.data.frame(seldat)) {
        dat <- seldat
        seldat <- NULL
				  ...
				  if(is.factor(dat[,j])) {
				    cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
				  	plots[[i]] <- ggplot(dat, aes_string(x=i, fill=j)) + geom_bar(position = "fill", alpha=.3) + 
				  		labs(list(y = "")) + scale_fill_manual(values=c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7"))
				  } else {
				    # compute lower and upper whiskers
				    plots[[i]] <- ggplot(dat, aes_string(x=i, y=j, fill=i,outline=FALSE)) + geom_boxplot(alpha = .3)
            ylim1 = boxplot.stats(dat$y)$stats[c(1, 5)]
				    ylim1
				    p1 = plots[[i]] + coord_cartesian(ylim = ylim1*1.05)
					  
					}
			  } else if(is.factor(dat[,j])) {
				  plots[[i]] <- ggplot(dat,outline=FALSE, aes_string(x=j, y=i, fill=j,outline=FALSE)) + geom_boxplot(alpha = .3) +
				 		coord_flip()
			  } else {
